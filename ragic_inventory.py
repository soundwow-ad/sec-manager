...
def parse_platform_region(raw_platform):
    ...
    # 判斷區域
    region = '未知'
    for r in ['全省', '北北基', '中彰投', '桃竹苗', '高高屏', '雲嘉南']:
...
def _extract_platform_from_sheet(df, sheet_name):
    ...
    region_keywords = ['全省', '北北基', '中彰投', '桃竹苗', '高高屏', '雲嘉南']
...
MOCK_PLATFORM_RAW = [
    "新鮮視全省", "新鮮視北北基", "新鮮視中彰投", "新鮮視桃竹苗", "新鮮視雲嘉南", "新鮮視高屏",
        ...
        for _, row in group.iterrows():
            try:
                ...
                segments.append({
                    ...
                    'seconds_type': row.get('seconds_type') or '銷售秒數',
        ...
        try:
            ...
            for d in date_range:
                ...
                daily_records.append({
                    ...
                    '媒體平台': seg.get('media_platform', '') or get_media_platform_display(seg.get('platform'), seg.get('channel'), ''),
                    '秒數用途': seg.get('seconds_type') or '銷售秒數',  # 年度總結表依類型分列用
                    '公司': seg['company'],
                    '業務': seg['sales'],
                    '客戶': seg['client'],
                    '產品': seg['product'],
...
def _segment_platform_display(seg):
    """表2 明細用平台顯示名稱（對齊 Excel：新鮮視北北基、全家廣播北北基、家樂福超市等）"""
...
def build_table2_summary_by_company(df_segments, df_daily, df_orders):
    """
    表2 區塊一：依公司統計總覽
...
def build_table2_details_by_company(df_segments, df_daily, df_orders):
    ...
    for company in seg['company'].dropna().unique():
        ...
        # 小計列
        sub = {'平台': '', '公司': company, '業務': '', '合約編號': '小計', '實收金額': detail['實收金額'].sum(),
...
def build_table3_monthly_control(df_daily, df_segments, custom_settings=None, year=None, month=None, monthly_capacity=None):
    ...
    for mp in media_platforms:
        ...
        if 'media_platform' in df_segments.columns:
            seg_mp = df_segments[df_segments['media_platform'] == mp].copy()
        else:
            seg_mp = df_segments[
                df_segments.apply(lambda r: get_media_platform_display(r.get('platform'), r.get('channel'), '') == mp, axis=1)
            ].copy()
        # 每日使用秒數（依媒體平台彙總）
...
def build_excel_table1_view(df_orders: pd.DataFrame, custom_settings=None, use_segments=True, df_segments=None) -> pd.DataFrame:
    ...
    def extract_region(p):
        if pd.isna(p):
            return ''
        p_str = str(p)
        for r in ['全省', '北北基', '桃竹苗', '中彰投', '高高屏', '雲嘉南']:
            if r in p_str:
                return r
        return ''
    ...
    if df_seg_t2.empty or df_daily.empty:
        ...
        try:
            ...
            with pd.ExcelWriter(buf, engine='openpyxl') as w:
                if not summary_t2.empty:
                    summary_t2.to_excel(w, sheet_name='依公司統計', index=False)
                for company_name, detail_df in details_t2.items():
